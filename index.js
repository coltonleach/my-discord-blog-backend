...
useEffect(() => {
  ...
  },
  {
    ...
    body: [
      ...
      `About three weeks ago I finished designing the front end and putting it together (that was annoying to learn, given the fact that it's not real CSS), so I moved on to getting the functionality to work and quickly came across a problem. I don't know how to do use authentication at all, or even where to begin. So that is where my research began. After I took a brief break from the project, of course.`,
      ...
      `A few days alter I started reading through the documentation and quickly realized that I'm not really understanding what I'm looking at (turns out I was looking at the wrong spot altogether), so I kept doing some research on other options out there. Because of the wrong impression I got from Supabase (entirely my fault), I decided to learn Firebase in the mean time on a sandbox project in the mean time, then consider using Supabase on the React Native app.`,
      `I eventually found a good video on integrating email/password user auth with Firebase and React, and went to town. It was an hour long, decided to take it slow to hopefully obsorb as much information as possible, so I did the first half one day and the second half the next. I quickly got the user auth working, and then tried figuring out how to store user data alongside it.`,
      `It took me way longer than I'm willing to admit to figure out how to store user data in Firestore, using the uid from Firebase's auth service, but I did it. My main problem was that almost every resource I came across that had examples and walk throughs would put every bit of code together, but the setup I had involved two seperate files: firebaseConfig.js that stores all the information and exports to auth and db, and then AuthContext.js that has all the backend functions like 'signin', 'signout', 'createNewUser', etc., and I really like that setup and vowed to figure out how to keep it that way.`,
